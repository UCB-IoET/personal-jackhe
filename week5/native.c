...
int contrib_osilate_gpio2(lua_State *L) {
    uint32_t volatile * toggle = (uint32_t volatile *) (0x400E1000 + 0x0 + 0x4C);
    uint32_t v = 1 << 16;
    while(1) {
        *toggle = v;
        *toggle = v;
...
const LUA_REG_TYPE contrib_native_map[] =
{
    ...
    { LSTRKEY( "osilate_gpio2"), LFUNCVAL ( contrib_osilate_gpio2 ) },
